...
function renderProjectsBucketDeck(model: ProjectsViewModel, lang: UiLang): string {
  const counts: Record<ProjectBucketKey, number> = {
    ...
    by_scenario: model.preset_groups.by_scenario.length,
    worth_trying_today: model.preset_groups.worth_trying_today.length,
    infra_tools: model.preset_groups.infra_tools.length,
    supplemental_inventory: model.preset_groups.supplemental_inventory.length,
  };
  return `
    ...
    stageTopHtml: `
      <div class="section-head weekly-matrix-shell-head projects-stage-head projects-stage-head-pagination-only">
        <div class="projects-stage-meta">
          <span class="projects-stage-count-label">
            ${escapeHtml(uiText(lang, "当前结果", "Visible"))}
            <span data-projects-count="true">${escapeHtml(String(initialWorkbench.totalCount))}</span>
            <span class="projects-stage-count-separator">/</span>
            ${escapeHtml(uiText(lang, "项目库总量", "Library Total"))}
            <span data-projects-total="true">${escapeHtml(String(model.projects.length))}</span>
          </span>
