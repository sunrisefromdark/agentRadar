...
- `RefactorSkill.md`：增量解耦重构、owner 迁移、新旧路径并行验证和删旧规则。
...
- 如果任务属于重构、解耦、业务 owner 迁移、旧逻辑替换或技术债治理，必须先读取 `RefactorSkill.md`；若进入代码落地，再按对应 exec-plan 读取 `CodeImplementation_Skill.md`。
