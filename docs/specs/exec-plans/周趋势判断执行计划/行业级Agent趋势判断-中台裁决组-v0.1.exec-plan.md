...
当前分支优先目标：把已就绪的政策金融 runtime、学术 formal dry-run、产品生态 Phase 6 replay/eval 输入并入中台 readiness/backlog；只生成 ref-only daily pack 与 rolling snapshot，不提前生成最终 weekly 三层输出。
...
| `Phase 3` normalization 与 fact snapshot | `Backlog ready` | `src/industry/platform/normalization/financePolicyDryRun.ts` 已消费政策金融 current bundle 与 runtime refs；`src/industry/platform/normalization/academicDryRun.ts` 已消费学术 formal current bundle；`src/industry/platform/normalization/productEcosystemDryRun.ts` 已消费产品生态 formal bundle；`src/industry/platform/audit/crossGroupIntegrationReadiness.ts` 已生成 `cross_group_integration_readiness`，并写出 ref-only `DailyIndustryEvidencePack.v2` 与 `RollingEvidenceWindowSnapshot.v1` 壳。 | 已可进入 cross-group replay/eval backlog 与 closed fact snapshot 准备；仍未完成 source-chain dedupe、fact resolution、owner arbitration、closed snapshot。 |
...
| `2号执行人` 学术前沿组 | 正式 `industry-signal-event-batch.v1` producer handoff；positive canonical、near-boundary、academic replay/eval refs。 | 当前 fixture 显示 `academic_formal_handoff_ready`，中台已可消费 formal current bundle 做 dry-run，并把 eval 资产排入 backlog。 |
...
| `4号执行人` 中台裁决组 | 任一组 refs 到齐后，先按组跑 contract test、registry runtime snapshot、normalization dry-run；已就绪三组先进入 readiness/backlog 与 ref-only daily/rolling 壳。 | 下一步推进 cross-group replay/eval backlog 与 closed fact snapshot；不能越过 audit / tier 决策直接脑补最终 weekly 三层输出。 |
