...
const SKILL_PATH = path.join(ROOT, "docs", "specs", "agent-work", "codeReviewSkill.md");
const RECEIPT_PATH = path.join(ROOT, "docs", "specs", "agent-work", "code-review-preflight.json");
const REVIEW_SCOPE = "code-review";
...
function checkReceipt(): void {
  ...
  if (receipt.skill_path !== expectedSkillPath) {
    throw new Error("codeReviewSkill.md path changed without refreshing the preflight receipt");
  }
  if (receipt.skill_sha256.toLowerCase() !== sha256(SKILL_PATH)) {
    throw new Error("codeReviewSkill.md changed without refreshing the preflight receipt");
  }
}
...
function main(): void {
  ...
  if (checkMode) {
    checkReceipt();
    console.log("[preflight] code review skill and receipt verified");
  }
}
