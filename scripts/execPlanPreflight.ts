...
const EXEC_PLAN_PATH = path.join(ROOT, "docs", "specs", "exec-plans", "agent-reach-external-discovery-and-evidence-v0.1.exec-plan.md");
...
function checkReceipt(): void {
  ...
  if (receipt.exec_plan_sha256.toLowerCase() !== sha256(EXEC_PLAN_PATH)) {
    throw new Error("exec-plan changed without refreshing the preflight receipt");
  }
}
