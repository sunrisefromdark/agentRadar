...
const SKILL_PATH = path.join(ROOT, "docs", "specs", "exec-plans", "ExecPlan_ReviewSkill.md");
const RECEIPT_PATH = path.join(ROOT, "docs", "specs", "exec-plans", "exec-plan-review-preflight.json");
const REVIEW_SCOPE = "exec-plan-review";
...
function checkReceipt(): void {
  ...
  if (receipt.skill_path !== expectedSkillPath) {
    throw new Error("ExecPlan_ReviewSkill.md path changed without refreshing the preflight receipt");
  }
  if (receipt.skill_sha256.toLowerCase() !== sha256(SKILL_PATH)) {
    throw new Error("ExecPlan_ReviewSkill.md changed without refreshing the preflight receipt");
  }
}
...
function main(): void {
  ...
  if (checkMode) {
    checkReceipt();
    console.log("[preflight] exec-plan review skill and receipt verified");
  }
}
