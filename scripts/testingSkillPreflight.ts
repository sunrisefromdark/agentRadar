...
const SKILL_PATH = path.join(ROOT, "docs", "specs", "agent-work", "TestingSkill.md");
const RECEIPT_PATH = path.join(ROOT, "docs", "specs", "agent-work", "testing-skill-preflight.json");
const REVIEW_SCOPE = "testing-skill";
...
function checkReceipt(): void {
  ...
  if (receipt.skill_path !== expectedSkillPath) {
    throw new Error("TestingSkill.md path changed without refreshing the preflight receipt");
  }
  if (receipt.skill_sha256.toLowerCase() !== sha256(SKILL_PATH)) {
    throw new Error("TestingSkill.md changed without refreshing the preflight receipt");
  }
}
...
function main(): void {
  ...
  if (checkMode) {
    checkReceipt();
    console.log("[preflight] testing skill and receipt verified");
  }
}
