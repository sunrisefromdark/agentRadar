...
export type ProductEcosystemHandoffBundle = {
...
export function validateProductEcosystemHandoff(
  registry: IndustrySchemaRegistry,
  ...
  for (const message of bundle.messages) {
    ...
    if (!ALLOWED_HANDOFF_SCHEMAS.has(message.payload_schema)) {
      return {
        ...
        message: `Unsupported product ecosystem handoff schema: ${message.payload_schema}`,
      };
    }
  ...
  for (const payload of bundle.payloads) {
    ...
    if (!ALLOWED_HANDOFF_SCHEMAS.has(payload.payload_schema)) {
      return {
        ok: false,
        reasonCode: "schema_mismatch",
        message: `Unsupported product ecosystem handoff schema: ${payload.payload_schema}`,
      };
    }
  ...
  const responsibilities = new Set(
    bundle.payloads.flatMap((payload) => [
      payload.responsibility_id,
      (payload.contribution as HandoffRecord | undefined)?.responsibility_id,
    ]).filter(hasText),
  );
  for (const responsibility of REQUIRED_RESPONSIBILITIES) {
    if (!responsibilities.has(responsibility)) {
      return {
        ...
        message: `Missing product ecosystem responsibility: ${responsibility}`,
      };
    }
  }
